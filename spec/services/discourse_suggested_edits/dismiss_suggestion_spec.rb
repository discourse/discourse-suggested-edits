...
RSpec.describe DiscourseSuggestedEdits::DismissSuggestion do
  ...
  describe ".call" do
    ...
    fab!(:acting_user) { Fabricate(:user, groups: [review_group]) }
    ...
    fab!(:suggested_edit) do
      Fabricate(:suggested_edit, post: post, user: suggester, base_post_version: post.version)
    end
    ...
    context "when user cannot review the suggested edit" do
    ...
    context "when the suggestion is not pending" do
      before { suggested_edit.update!(status: :withdrawn) }
    ...
    context "when everything is ok" do
      ...
      it "dismisses the suggested edit" do
        expect { result }.to change { suggested_edit.reload.status }.from("pending").to("dismissed")
      end
